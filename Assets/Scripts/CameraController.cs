...
public class CameraController : MonoBehaviour
{
	...
	bool CheckXMargin ()
	{
		return Mathf.Abs (transform.position.x - player.position.x) > m_xMargin;
		//return true;
	}

	bool CheckYMargin ()
	{
		return Mathf.Abs (transform.position.y - player.position.y) > m_yMargin;
	...
	void TrackPlayer ()
	{
		...
		if (CheckXMargin ()) {
			targetX = Mathf.Lerp (transform.position.x, player.position.x, Time.deltaTime * m_xSmooth);
		}

		if (CheckYMargin ()) {
			targetY = Mathf.Lerp (transform.position.y, player.position.y + m_yOffset, Time.deltaTime * m_ySmooth);
		}
